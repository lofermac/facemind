...
export default function CadastroPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 2. A Mágica do Supabase Auth
      const { data, error } = await supabase.auth.signUp({
        email: formData.email,
        password: formData.senha,
        options: {
          // Passa dados extras que serão usados no trigger
          data: {
            nome: formData.nomeCompleto,
            whatsapp: formData.whatsapp,
            status_plano: 'trialing' // Período de teste gratuito
          }
        }
      });
      ...
      <section className="relative z-10 pt-24 pb-8 px-8 overflow-hidden min-h-screen flex items-center">
        ...
        <div className="max-w-md mx-auto w-full">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Nome Completo */}
            <div>
              <label htmlFor="nomeCompleto" className="block text-sm font-medium text-gray-300 mb-1.5">
                Nome Completo
              </label>
              <input
                type="text"
                ...
                className="w-full px-3 py-2.5 bg-gray-900/50 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            {/* Confirmar Senha */}
            <div>
              <label htmlFor="confirmarSenha" className="block text-sm font-medium text-gray-300 mb-1.5">
                Confirmar Senha
              </label>
              <div className="relative">
                <input
            ...
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full px-6 py-3 rounded-lg text-base font-medium transition-all duration-300 mt-6 ${
                isLoading 
                  ? 'bg-gray-600 cursor-not-allowed' 
                  : 'bg-blue-600 hover:bg-blue-500 hover:scale-105 hover:shadow-[0_0_30px_rgba(59,130,246,0.4)]'
              } text-white`}
            >
            ...
            <div className="text-center mt-4">
              <p className="text-gray-400 text-sm">
                Já tem uma conta?{" "}
                <Link href="/profissionais/login" className="text-blue-400 hover:text-blue-300 transition-colors font-medium">
                  Faça login
                </Link>
              </p>
            </div>
