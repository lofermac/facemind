    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-6 md:flex md:items-center md:justify-between">
        ...
        <div className="mt-4 flex md:mt-0 md:ml-4">
          <button onClick={() => setShowNovoPacienteModal(true)} className="inline-flex items-center px-5 py-3 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-gradient-to-r from-blue-500 to-blue-700 hover:from-blue-600 hover:to-blue-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition duration-300 ease-in-out transform hover:scale-105 hover:shadow-lg cursor-pointer">
      ...
      <div className="mb-6 flex space-x-4">
        <input
          ...
          className="shadow-sm focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-base border-gray-300 rounded-lg p-3 transition duration-150 ease-in-out"
          value={filtroNome}
          onChange={(e) => setFiltroNome(e.target.value)}
        />
        <select
          className="shadow-sm focus:ring-blue-500 focus:border-blue-500 block sm:text-base border-gray-300 rounded-lg p-3 transition duration-150 ease-in-out bg-white"
          value={filtroStatus}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {pacientesParaLista.map((paciente) => (
          <div key={paciente.id} className="relative bg-white shadow rounded-lg p-6 transition-transform transform hover:scale-105 hover:shadow-lg">
            <div className="flex justify-between items-start mb-2">
              <div className="flex items-center space-x-2">
                <Link 
                  href={`/procedimentos?filtroNome=${encodeURIComponent(paciente.nome)}`}
                  className="text-md font-bold text-gray-900 hover:text-blue-600 cursor-pointer"
                >
                  {paciente.nome}
                </Link>
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium ${getStatusColor(paciente.status_calculado)}`}>{paciente.status_calculado}</span>
              </div>
            ...
            <p className="text-sm text-gray-500 mb-1"><strong>Idade:</strong> {calcularIdade(paciente.data_nascimento)}</p>
            <div className="flex justify-between items-center">
              <p className="text-sm text-gray-500 mb-1"><strong>WhatsApp:</strong> {formatarWhatsApp(paciente.whatsapp)}</p>
              <a href={`https://wa.me/+55${paciente.whatsapp?.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="text-green-500 hover:text-green-600">
            ...
            <p className="text-sm text-gray-500 mb-1"><strong>Data de Criação:</strong> {new Date(paciente.created_at).toLocaleDateString('pt-BR')}</p>
          </div>
        ))}
      </div>
      ...
      {showNovoPacienteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-white bg-opacity-70">
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full mx-4">
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Cadastrar Novo Paciente</h3>
